...
This directory contains a proof-of-concept application that demonstrates how to use `acme.cr` to automatically provision an SSL certificate and serve a secure webpage.
...
1.  Build the example:
    ```bash
    crystal build examples/server.cr -o server
    ```

2.  Run the server (replace with your domain and email):
    ```bash
    sudo ./server -d example.com -e admin@example.com
    ```
...
3.  To use the **Production** environment (trusted certificate):
    ```bash
    sudo ./server -d example.com -e admin@example.com --prod
    ```
